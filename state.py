...
class StateManager:
    ...
    def upsert_target(self, program: str, name: str, base_domain: str,
                      ...
                      source: str = "scope", discovered_from: int = None) -> Optional[int]:
        now = datetime.utcnow().isoformat()
        try:
            cur = self._conn.execute(
                """INSERT INTO targets(program, platform, name, base_domain, is_wildcard,
                                       category, source, discovered_from, last_seen)
                   VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                (program, platform, name, base_domain, int(is_wildcard),
                 category, source, discovered_from, now),
            )
            row = cur.fetchone()
            self._conn.commit()
            if row:
    ...
    def get_targets(self, program: str = None, platform: str = None,
                    ...
                    excluded_programs: bool = False) -> list[sqlite3.Row]:
        query = """
    ...
    def queue_scan(self, target_id: int, tool: str, force: bool = False) -> int:
        """
