...
def sync(ctx, platform, program):
        ...
        if "bugcrowd" in platforms:
            ...
            if program:
                ...
                for prog in bc.iter_programs():
                    ...
                    total_programs += 1
                    total_targets += len([t for t in prog.targets if t.in_scope])
                    total_out_of_scope += len([t for t in prog.targets if not t.in_scope])
    ...
    if not program:
        console.print(f"[green]Done. Synced {total_programs} programs, {total_targets} in-scope targets. Recorded {total_out_of_scope} out-of-scope targets for explicit exclusion.[/]")
...
def _save_program(state: StateManager, prog: Program):
    ...
    for t in prog.targets:
        state.upsert_target(
            ...
            is_wildcard=t.is_wildcard, category=t.category, platform=t.platform, in_scope=t.in_scope,
        )
...
def list_cmd(ctx, platform, program, show_targets, wildcards, discovered):
    ...
    if show_targets or program:
        ...
        table.add_column("In Scope", style="bold")
        for t in targets:
            if t["is_wildcard"]:
                ...
                ttype = "scope"
            in_scope_str = "[green]YES[/]" if t["in_scope"] else "[red]NO[/]"
            table.add_row(t["platform"], t["program"], t["name"], ttype, in_scope_str)
        console.print(table)
