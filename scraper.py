...
BC_PROGRAMS_API = "https://bugcrowd.com/programs.json"
BC_SCOPE_API = "https://bugcrowd.com/{program}/target_groups.json"
...
class BugcrowdScraper:
    ...
    def iter_programs(self) -> Generator[Program, None, None]:
        ...
        while True:
            ...
            if not programs:
                break
            for p in programs:
                code = p.get("program_slug") or p.get("code") or p.get("handle")
    ...
    def fetch_targets(self, program: Program) -> list[ScopeTarget]:
        targets = []
        url = BC_SCOPE_API.format(program=program.code)
        data = self._get(url)
        if not data or not isinstance(data, dict):
            return targets
